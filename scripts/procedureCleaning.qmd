...
proc_filepath = "originals/IMPC_procedure.csv"
proc_file = fread(str_c(base_path, proc_filepath))
head(proc_file[,-2],3) # dont show description for ease-of-viewing below
```
...
head(collapsed_df[,-2], 3) # dont show description for ease-of-viewing below
```
...
# and the cleaned parameter descriptions table (and is still present
# in the collapsed_df)
final_collapsed_df = collapsed_df[,-4] # critically, copy here
head(final_collapsed_df[,-2],3)
```
...
new_filepath = "output/IMPC_procedure_clean.csv"
fwrite(final_collapsed_df, str_c(base_path, new_filepath))
```

```{r}
# read in the cleaned IMPC_param_desc CSV to add column with the "procedure_group_id"
param_descs_clean_filepath = "output/IMPC_procedure_params.csv"
# param_descs_clean_filepath = "output/IMPC_parameter_description_clean.csv"
param_descs_clean = fread(str_c(base_path, param_descs_clean_filepath))
...
fwrite(param_descs_clean, str_c(base_path, param_descs_clean_filepath))
```

```{r}
param_descs_clean[param_descs_clean$parameter_id == "IMPC_GRS_001_001",]
```
