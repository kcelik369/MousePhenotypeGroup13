...
1. **procedureCleaning.qmd** : eliminated many redundant rows by reforming relation with IMPC parameters to reference "procedure groups" which cover a range of IMPC parameters. Adds a new column to the `impc_procedure_params` table created just previously to maintain connectivity.
1. **geneTable.qmd** : Created from the gene-related rows of the merged analyses table, used to link from there to the table of diseases. Redundant storage of gene symbol information; that column was left in the merged analyses table to maintain parity with the CSV referenced by the R Shiny app.
...
1. **ElbowMethod.R** : Demonstrates rationale for default cluster number value during PCA analysis in the below R Shiny app. Run only with IMPC parameters.
1. **final_rshiny_app.R** : Also run only with IMPC parameters. Interactive R Shiny dashboard to show mouse phenotypic data from the IMPC in three different ways: 
